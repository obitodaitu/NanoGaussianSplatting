...
void FGlobalSplatBufferManager::Release()
{
	...
	// Cluster-based CalcViewData buffers
	...
	TestCalcDistIndirectArgs.SafeRelease();
	TestCalcDistIndirectArgsUAV.SafeRelease();
	TestSortIndirectArgs.SafeRelease();
	TestSortIndirectArgsUAV.SafeRelease();
	TestSortParams.SafeRelease();
	TestSortParamsUAV.SafeRelease();
	TestDrawIndirectArgs.SafeRelease();
	TestDrawIndirectArgsUAV.SafeRelease();
	ShadowClusterViewDataBuffer.SafeRelease();
	ShadowClusterViewDataBufferUAV.SafeRelease();
...
// ============================================================================
// Cluster-based CalcViewData: EnsureClusterWorkListBuffers
// ============================================================================

void FGlobalSplatBufferManager::EnsureClusterWorkListBuffers(FRHICommandListImmediate& RHICmdList)
{
	if (bClusterWorkListBuffersAllocated)
	{
		return;
	}
	...
	VisibleClusterWorkList = CreateStructuredBuffer(RHICmdList, TEXT("VisibleClusterWorkList"), 16, MaxClusters);
	VisibleClusterWorkListUAV = RHICmdList.CreateUnorderedAccessView(
		VisibleClusterWorkList,
		FRHIViewDesc::CreateBufferUAV().SetType(FRHIViewDesc::EBufferType::Structured).SetStride(16));
	VisibleClusterWorkListSRV = RHICmdList.CreateShaderResourceView(
		VisibleClusterWorkList,
		FRHIViewDesc::CreateBufferSRV().SetType(FRHIViewDesc::EBufferType::Structured).SetStride(16));

	// VisibleClusterCountBuffer: single uint32
	VisibleClusterCountBuffer = CreateStructuredBuffer(RHICmdList, TEXT("VisibleClusterCountBuffer"), UintStride, 1);
	VisibleClusterCountBufferUAV = RHICmdList.CreateUnorderedAccessView(
		VisibleClusterCountBuffer,
		FRHIViewDesc::CreateBufferUAV().SetType(FRHIViewDesc::EBufferType::Structured).SetStride(UintStride));
	...
	ClusterSplatCountsBuffer = CreateStructuredBuffer(RHICmdList, TEXT("ClusterSplatCountsBuffer"), UintStride, MaxClusters);
	ClusterSplatCountsBufferUAV = RHICmdList.CreateUnorderedAccessView(
		ClusterSplatCountsBuffer,
		FRHIViewDesc::CreateBufferUAV().SetType(FRHIViewDesc::EBufferType::Structured).SetStride(UintStride));
	ClusterSplatCountsBufferSRV = RHICmdList.CreateShaderResourceView(
		ClusterSplatCountsBuffer,
		FRHIViewDesc::CreateBufferSRV().SetType(FRHIViewDesc::EBufferType::Structured).SetStride(UintStride));

	bClusterWorkListBuffersAllocated = true;

	UE_LOG(LogTemp, Log, TEXT("[GS Cluster] Work list buffers allocated: MaxClusters=%u"), MaxClusters);
}

// ============================================================================
// Cluster-based CalcViewData: DispatchBuildVisibleClusterWorkList
// ============================================================================
...
void FGlobalSplatBufferManager::DispatchBuildVisibleClusterWorkList(
	FRHICommandListImmediate& RHICmdList,
	const TArray<FGaussianSplatSceneProxy*>& ValidProxies)
{
	if (!IsReady() || TotalLeafClusterCount == 0 || GetProxyCount() == 0)
	...
	// Stage 2 shadow buffers must exist (DispatchGlobalClusterCulling must run first)
	if (!bShadowBuffersAllocated)
	{
		return;
	}

	SCOPED_DRAW_EVENT(RHICmdList, GaussianSplatBuildVisibleClusterWorkList);

	// Ensure cluster work list buffers exist
	EnsureClusterWorkListBuffers(RHICmdList);

	TShaderMapRef<FBuildVisibleClusterWorkListResetCS> ResetShader(GetGlobalShaderMap(GMaxRHIFeatureLevel));
	TShaderMapRef<FBuildVisibleClusterWorkListCS> LeafShader(GetGlobalShaderMap(GMaxRHIFeatureLevel));
	TShaderMapRef<FBuildVisibleClusterWorkListLODCS> LODShader(GetGlobalShaderMap(GMaxRHIFeatureLevel));

	if (!ResetShader.IsValid() || !LeafShader.IsValid() || !LODShader.IsValid())
	{
		UE_LOG(LogTemp, Warning, TEXT("[GS Cluster] BuildVisibleClusterWorkList shaders not valid"));
		return;
	}

	// Transition work list buffers to UAVCompute
	RHICmdList.Transition(FRHITransitionInfo(VisibleClusterWorkList, ERHIAccess::Unknown, ERHIAccess::UAVCompute));
	RHICmdList.Transition(FRHITransitionInfo(VisibleClusterCountBuffer, ERHIAccess::Unknown, ERHIAccess::UAVCompute));
	RHICmdList.Transition(FRHITransitionInfo(ClusterSplatCountsBuffer, ERHIAccess::Unknown, ERHIAccess::UAVCompute));

	// Shadow bitmaps should already be SRVCompute from DispatchGlobalClusterCulling
	RHICmdList.Transition(FRHITransitionInfo(ShadowClusterVisibilityBitmap, ERHIAccess::Unknown, ERHIAccess::SRVCompute));
	RHICmdList.Transition(FRHITransitionInfo(ShadowSelectedClusterBuffer, ERHIAccess::Unknown, ERHIAccess::SRVCompute));
	RHICmdList.Transition(FRHITransitionInfo(GlobalClusterBuffer, ERHIAccess::Unknown, ERHIAccess::SRVCompute));
	RHICmdList.Transition(FRHITransitionInfo(ProxyMetadataBuffer, ERHIAccess::Unknown, ERHIAccess::SRVCompute));
	...
	// Step 1: Reset VisibleClusterCount to 0
	{
		FBuildVisibleClusterWorkListResetCS::FParameters ResetParams;
		ResetParams.VisibleClusterCountBuffer = VisibleClusterCountBufferUAV;
	...
	// UAV barrier: reset → leaf dispatch
	RHICmdList.Transition(FRHITransitionInfo(VisibleClusterCountBuffer, ERHIAccess::UAVCompute, ERHIAccess::UAVCompute));

	// Step 2: Leaf clusters
	{
		FBuildVisibleClusterWorkListCS::FParameters LeafParams;
		LeafParams.GlobalClusterBuffer = GlobalClusterBufferSRV;
		LeafParams.ProxyMetadataBuffer = ProxyMetadataBufferSRV;
		LeafParams.ShadowClusterVisibilityBitmap = ShadowClusterVisibilityBitmapSRV;
		LeafParams.ShadowSelectedClusterBuffer = ShadowSelectedClusterBufferSRV;
		LeafParams.VisibleClusterWorkList = VisibleClusterWorkListUAV;
		LeafParams.VisibleClusterCountBuffer = VisibleClusterCountBufferUAV;
		LeafParams.ClusterSplatCountsBuffer = ClusterSplatCountsBufferUAV;
		LeafParams.ProxyCount = GetProxyCount();
		LeafParams.TotalLeafClusterCount = TotalLeafClusterCount;
		LeafParams.UseLODRendering = 1;

		const uint32 NumGroups = FMath::DivideAndRoundUp(TotalLeafClusterCount, 64u);

		SetComputePipelineState(RHICmdList, LeafShader.GetComputeShader());
		SetShaderParameters(RHICmdList, LeafShader, LeafShader.GetComputeShader(), LeafParams);
		RHICmdList.DispatchComputeShader(NumGroups, 1, 1);
		UnsetShaderUAVs(RHICmdList, LeafShader, LeafShader.GetComputeShader());
	}

	// UAV barrier: leaf → LOD dispatch
	RHICmdList.Transition(FRHITransitionInfo(VisibleClusterCountBuffer, ERHIAccess::UAVCompute, ERHIAccess::UAVCompute));
	RHICmdList.Transition(FRHITransitionInfo(VisibleClusterWorkList, ERHIAccess::UAVCompute, ERHIAccess::UAVCompute));
	RHICmdList.Transition(FRHITransitionInfo(ClusterSplatCountsBuffer, ERHIAccess::UAVCompute, ERHIAccess::UAVCompute));

	// Step 3: LOD parent clusters
	{
		// ShadowLODClusterBuffer needs SRV (no permanent SRV exists)
		RHICmdList.Transition(FRHITransitionInfo(ShadowLODClusterBuffer, ERHIAccess::Unknown, ERHIAccess::SRVCompute));
		RHICmdList.Transition(FRHITransitionInfo(ShadowLODClusterCountBuffer, ERHIAccess::Unknown, ERHIAccess::SRVCompute));

		FShaderResourceViewRHIRef ShadowLODClusterBufferSRV = RHICmdList.CreateShaderResourceView(
			ShadowLODClusterBuffer,
			FRHIViewDesc::CreateBufferSRV().SetType(FRHIViewDesc::EBufferType::Structured).SetStride(sizeof(uint32)));
		FShaderResourceViewRHIRef ShadowLODClusterCountBufferSRV = RHICmdList.CreateShaderResourceView(
			ShadowLODClusterCountBuffer,
			FRHIViewDesc::CreateBufferSRV().SetType(FRHIViewDesc::EBufferType::Structured).SetStride(sizeof(uint32)));

		FBuildVisibleClusterWorkListLODCS::FParameters LODParams;
		LODParams.GlobalClusterBuffer = GlobalClusterBufferSRV;
		LODParams.ProxyMetadataBuffer = ProxyMetadataBufferSRV;
		LODParams.ShadowLODClusterBuffer = ShadowLODClusterBufferSRV;
		LODParams.ShadowLODClusterCountBuffer = ShadowLODClusterCountBufferSRV;
		LODParams.VisibleClusterWorkList = VisibleClusterWorkListUAV;
		LODParams.VisibleClusterCountBuffer = VisibleClusterCountBufferUAV;
		LODParams.ClusterSplatCountsBuffer = ClusterSplatCountsBufferUAV;
		LODParams.ProxyCount = GetProxyCount();
		LODParams.TotalClusterCount = TotalClusterCount;

		// Conservative dispatch: TotalClusterCount / 64 (shader early-outs if lodIdx >= LODClusterCount)
		const uint32 NumGroups = FMath::DivideAndRoundUp(TotalClusterCount, 64u);

		SetComputePipelineState(RHICmdList, LODShader.GetComputeShader());
		SetShaderParameters(RHICmdList, LODShader, LODShader.GetComputeShader(), LODParams);
		RHICmdList.DispatchComputeShader(NumGroups, 1, 1);
		UnsetShaderUAVs(RHICmdList, LODShader, LODShader.GetComputeShader());
	}

	// Transition outputs to SRV for downstream
	RHICmdList.Transition(FRHITransitionInfo(VisibleClusterWorkList, ERHIAccess::UAVCompute, ERHIAccess::SRVCompute));
	RHICmdList.Transition(FRHITransitionInfo(VisibleClusterCountBuffer, ERHIAccess::UAVCompute, ERHIAccess::SRVCompute));
	RHICmdList.Transition(FRHITransitionInfo(ClusterSplatCountsBuffer, ERHIAccess::UAVCompute, ERHIAccess::SRVCompute));

	// ---------------------------------------------------------------
	// GPU Readback Test (triggered by gs.ClusterWorkListTest CVar)
	// Uses FRHIGPUBufferReadback to enqueue copies this frame,
	// then polls on next frame(s) until data is ready.
	// State tracked via bClusterTestWaitingForReadback (not CVar,
	// because CVars can't be set from the render thread).
	// ---------------------------------------------------------------

	int32 TestVal = CVarClusterWorkListTest.GetValueOnRenderThread();

	// Phase 1: CVar is 1 and not already waiting → enqueue readbacks
	if (TestVal == 1 && !bClusterTestWaitingForReadback)
	{
		bClusterTestWaitingForReadback = true;

		// Allocate readback objects (stored as members for multi-frame polling)
		ClusterTest_CountReadback = MakeUnique<FRHIGPUBufferReadback>(TEXT("ClusterTestCountReadback"));
		ClusterTest_SplatCountsReadback = MakeUnique<FRHIGPUBufferReadback>(TEXT("ClusterTestSplatCountsReadback"));
		ClusterTest_OldVisibleReadback = MakeUnique<FRHIGPUBufferReadback>(TEXT("ClusterTestOldVisibleReadback"));
		ClusterTest_WorkItemsReadback = MakeUnique<FRHIGPUBufferReadback>(TEXT("ClusterTestWorkItemsReadback"));

		// Enqueue copy: VisibleClusterCount (1 uint32)
		RHICmdList.Transition(FRHITransitionInfo(VisibleClusterCountBuffer, ERHIAccess::SRVCompute, ERHIAccess::CopySrc));
		ClusterTest_CountReadback->EnqueueCopy(RHICmdList, VisibleClusterCountBuffer, sizeof(uint32));

		// Enqueue copy: ClusterSplatCounts (up to TotalClusterCount uint32s)
		uint32 MaxClusters = FMath::Max(TotalClusterCount, 1u);
		ClusterTest_MaxReadbackClusters = MaxClusters;
		RHICmdList.Transition(FRHITransitionInfo(ClusterSplatCountsBuffer, ERHIAccess::SRVCompute, ERHIAccess::CopySrc));
		ClusterTest_SplatCountsReadback->EnqueueCopy(RHICmdList, ClusterSplatCountsBuffer, MaxClusters * sizeof(uint32));

		// Enqueue copy: Old path ShadowVisibleCountArray
		ClusterTest_ProxyCount = GetProxyCount();
		if (bShadowCompactBuffersAllocated && ShadowVisibleCountArray.IsValid())
		{
			RHICmdList.Transition(FRHITransitionInfo(ShadowVisibleCountArray, ERHIAccess::Unknown, ERHIAccess::CopySrc));
			ClusterTest_OldVisibleReadback->EnqueueCopy(RHICmdList, ShadowVisibleCountArray, ClusterTest_ProxyCount * sizeof(uint32));
		}

		// Enqueue copy: WorkItems (first 10 for debug, 16 bytes each)
		uint32 DebugItems = FMath::Min(MaxClusters, 10u);
		RHICmdList.Transition(FRHITransitionInfo(VisibleClusterWorkList, ERHIAccess::SRVCompute, ERHIAccess::CopySrc));
		ClusterTest_WorkItemsReadback->EnqueueCopy(RHICmdList, VisibleClusterWorkList, DebugItems * 16);
	}
	// Phase 2: Poll for readback completion
	else if (bClusterTestWaitingForReadback)
	{
		if (ClusterTest_CountReadback && ClusterTest_CountReadback->IsReady()
			&& ClusterTest_SplatCountsReadback && ClusterTest_SplatCountsReadback->IsReady()
			&& ClusterTest_OldVisibleReadback && ClusterTest_OldVisibleReadback->IsReady())
		{
			// Read VisibleClusterCount
			uint32 ReadVisibleClusterCount = *(const uint32*)ClusterTest_CountReadback->Lock(sizeof(uint32));
			ClusterTest_CountReadback->Unlock();

			// Sum ClusterSplatCounts
			uint32 SumSplatCounts = 0;
			if (ReadVisibleClusterCount > 0)
			{
				uint32 ReadCount = FMath::Min(ReadVisibleClusterCount, ClusterTest_MaxReadbackClusters);
				const uint32* Counts = (const uint32*)ClusterTest_SplatCountsReadback->Lock(ReadCount * sizeof(uint32));
				for (uint32 i = 0; i < ReadCount; i++)
				{
					SumSplatCounts += Counts[i];
				}
				ClusterTest_SplatCountsReadback->Unlock();
			}

			// Sum old path visible counts
			uint32 OldTotalVisible = 0;
			if (ClusterTest_ProxyCount > 0)
			{
				const uint32* OldCounts = (const uint32*)ClusterTest_OldVisibleReadback->Lock(ClusterTest_ProxyCount * sizeof(uint32));
				for (uint32 i = 0; i < ClusterTest_ProxyCount; i++)
				{
					OldTotalVisible += OldCounts[i];
				}
				ClusterTest_OldVisibleReadback->Unlock();
			}

			bool bPass = (SumSplatCounts == OldTotalVisible);
			UE_LOG(LogTemp, Log,
				TEXT("[GS Cluster] Stage1 TEST: VisibleClusters=%u, NewSum=%u vs OldTotalVisible=%u -> %s"),
				ReadVisibleClusterCount, SumSplatCounts, OldTotalVisible,
				bPass ? TEXT("PASS") : TEXT("FAIL <<<"));

			if (!bPass && ReadVisibleClusterCount > 0 && ClusterTest_WorkItemsReadback->IsReady())
			{
				uint32 DebugCount = FMath::Min(ReadVisibleClusterCount, 10u);
				const FVisibleClusterWorkItem* Items = (const FVisibleClusterWorkItem*)ClusterTest_WorkItemsReadback->Lock(DebugCount * 16);
				...
				ClusterTest_WorkItemsReadback->Unlock();
			}

			// Cleanup
			ClusterTest_CountReadback.Reset();
			ClusterTest_SplatCountsReadback.Reset();
			ClusterTest_OldVisibleReadback.Reset();
			ClusterTest_WorkItemsReadback.Reset();
			bClusterTestWaitingForReadback = false;
			// Note: CVar stays at 1 until user manually sets to 0.
			// Re-running just requires setting it to 0 then 1 again.
		}
		// else: not ready yet, will poll again next frame
	}
}

// ============================================================================
// Cluster-based CalcViewData: DispatchClusterPrefixSum
// ============================================================================

static TAutoConsoleVariable<int32> CVarClusterPrefixSumTest(
	TEXT("gs.ClusterPrefixSumTest"), 0,
	TEXT("Set to 1 to test cluster prefix sum (compares TotalVisible and indirect args with old path)"));
	...
	uint32 MaxRenderBudget)
{
	...
	SCOPED_DRAW_EVENT(RHICmdList, GaussianSplatClusterPrefixSum);

	TShaderMapRef<FClusterPrefixSumCS> PrefixSumShader(GetGlobalShaderMap(GMaxRHIFeatureLevel));
	if (!PrefixSumShader.IsValid())
	{
		UE_LOG(LogTemp, Warning, TEXT("[GS Cluster] ClusterPrefixSum shader not valid"));
		return;
	}
	...
	// Allocate ClusterOutputOffsetsBuffer if needed (TotalClusterCount + 1)
	if (!ClusterOutputOffsetsBuffer.IsValid())
	{
		ClusterOutputOffsetsBuffer = CreateStructuredBuffer(RHICmdList, TEXT("ClusterOutputOffsetsBuffer"), UintStride, MaxClusters + 1);
		ClusterOutputOffsetsBufferUAV = RHICmdList.CreateUnorderedAccessView(
			ClusterOutputOffsetsBuffer,
			FRHIViewDesc::CreateBufferUAV().SetType(FRHIViewDesc::EBufferType::Structured).SetStride(UintStride));
		ClusterOutputOffsetsBufferSRV = RHICmdList.CreateShaderResourceView(
			ClusterOutputOffsetsBuffer,
			FRHIViewDesc::CreateBufferSRV().SetType(FRHIViewDesc::EBufferType::Structured).SetStride(UintStride));
	}

	// Allocate ClusterCalcViewDataIndirectArgs if needed (3 uint32s)
	if (!ClusterCalcViewDataIndirectArgs.IsValid())
	{
		ClusterCalcViewDataIndirectArgs = CreateStructuredBuffer(RHICmdList, TEXT("ClusterCalcViewDataIndirectArgs"), UintStride, 3);
		ClusterCalcViewDataIndirectArgsUAV = RHICmdList.CreateUnorderedAccessView(
			ClusterCalcViewDataIndirectArgs,
			FRHIViewDesc::CreateBufferUAV().SetType(FRHIViewDesc::EBufferType::Structured).SetStride(UintStride));
	}

	// Allocate test indirect args buffers
	if (!TestCalcDistIndirectArgs.IsValid())
	{
		TestCalcDistIndirectArgs = CreateStructuredBuffer(RHICmdList, TEXT("TestCalcDistIndirectArgs"), UintStride, 3);
		TestCalcDistIndirectArgsUAV = RHICmdList.CreateUnorderedAccessView(
			TestCalcDistIndirectArgs,
			FRHIViewDesc::CreateBufferUAV().SetType(FRHIViewDesc::EBufferType::Structured).SetStride(UintStride));

		TestSortIndirectArgs = CreateStructuredBuffer(RHICmdList, TEXT("TestSortIndirectArgs"), UintStride, 3);
		TestSortIndirectArgsUAV = RHICmdList.CreateUnorderedAccessView(
			TestSortIndirectArgs,
			FRHIViewDesc::CreateBufferUAV().SetType(FRHIViewDesc::EBufferType::Structured).SetStride(UintStride));

		TestSortParams = CreateStructuredBuffer(RHICmdList, TEXT("TestSortParams"), UintStride, 2);
		TestSortParamsUAV = RHICmdList.CreateUnorderedAccessView(
			TestSortParams,
			FRHIViewDesc::CreateBufferUAV().SetType(FRHIViewDesc::EBufferType::Structured).SetStride(UintStride));

		TestDrawIndirectArgs = CreateStructuredBuffer(RHICmdList, TEXT("TestDrawIndirectArgs"), UintStride, 5);
		TestDrawIndirectArgsUAV = RHICmdList.CreateUnorderedAccessView(
			TestDrawIndirectArgs,
			FRHIViewDesc::CreateBufferUAV().SetType(FRHIViewDesc::EBufferType::Structured).SetStride(UintStride));
	}

	// Transition inputs to SRVCompute (should already be from Stage 1)
	RHICmdList.Transition(FRHITransitionInfo(ClusterSplatCountsBuffer, ERHIAccess::Unknown, ERHIAccess::SRVCompute));
	RHICmdList.Transition(FRHITransitionInfo(VisibleClusterCountBuffer, ERHIAccess::Unknown, ERHIAccess::SRVCompute));
	...
	// Dispatch
	{
		FClusterPrefixSumCS::FParameters Params;
		Params.ClusterSplatCountsBuffer = ClusterSplatCountsBufferSRV;
	...
	// Transition outputs to SRVCompute for downstream
	RHICmdList.Transition(FRHITransitionInfo(ClusterOutputOffsetsBuffer, ERHIAccess::UAVCompute, ERHIAccess::SRVCompute));
	RHICmdList.Transition(FRHITransitionInfo(ClusterCalcViewDataIndirectArgs, ERHIAccess::UAVCompute, ERHIAccess::SRVCompute));

	// ---------------------------------------------------------------
	// GPU Readback Test (triggered by gs.ClusterPrefixSumTest CVar)
	// Self-consistency: compare TestDrawIndirectArgs[1] (TotalVisible
	// from prefix sum) against sum(ClusterSplatCounts) from same frame.
	// Stage 1 already proved sum(ClusterSplatCounts) == OldTotalVisible.
	// NOTE: We can't compare against old path's GlobalDrawIndirectArgs
	// here because it hasn't been written yet (old PrefixSum runs later).
	// ---------------------------------------------------------------
	int32 TestVal = CVarClusterPrefixSumTest.GetValueOnRenderThread();

	if (TestVal == 1 && !bPrefixTestWaitingForReadback)
	{
		bPrefixTestWaitingForReadback = true;

		// Read new DrawIndirectArgs (5 uint32s — [1] = TotalVisible from prefix sum)
		PrefixTest_NewDrawArgsReadback = MakeUnique<FRHIGPUBufferReadback>(TEXT("PrefixTestNewDrawArgs"));
		RHICmdList.Transition(FRHITransitionInfo(TestDrawIndirectArgs, ERHIAccess::UAVCompute, ERHIAccess::CopySrc));
		PrefixTest_NewDrawArgsReadback->EnqueueCopy(RHICmdList, TestDrawIndirectArgs, 5 * UintStride);

		// Read VisibleClusterCount + ClusterSplatCounts for independent sum
		PrefixTest_NewTotalReadback = MakeUnique<FRHIGPUBufferReadback>(TEXT("PrefixTestVisClusterCount"));
		RHICmdList.Transition(FRHITransitionInfo(VisibleClusterCountBuffer, ERHIAccess::Unknown, ERHIAccess::CopySrc));
		PrefixTest_NewTotalReadback->EnqueueCopy(RHICmdList, VisibleClusterCountBuffer, UintStride);

		PrefixTest_OldDrawArgsReadback = MakeUnique<FRHIGPUBufferReadback>(TEXT("PrefixTestSplatCounts"));
		RHICmdList.Transition(FRHITransitionInfo(ClusterSplatCountsBuffer, ERHIAccess::Unknown, ERHIAccess::CopySrc));
		uint32 MaxReadClusters = FMath::Max(TotalClusterCount, 1u);
		PrefixTest_OldDrawArgsReadback->EnqueueCopy(RHICmdList, ClusterSplatCountsBuffer, MaxReadClusters * UintStride);
		ClusterTest_MaxReadbackClusters = MaxReadClusters; // reuse Stage 1 member
	}
	else if (bPrefixTestWaitingForReadback)
	{
		if (PrefixTest_NewDrawArgsReadback && PrefixTest_NewDrawArgsReadback->IsReady()
			&& PrefixTest_NewTotalReadback && PrefixTest_NewTotalReadback->IsReady()
			&& PrefixTest_OldDrawArgsReadback && PrefixTest_OldDrawArgsReadback->IsReady())
		{
			// PrefixSum TotalVisible from DrawIndirectArgs[1]
			const uint32* NewArgs = (const uint32*)PrefixTest_NewDrawArgsReadback->Lock(5 * UintStride);
			uint32 PrefixSumTotal = NewArgs[1];
			PrefixTest_NewDrawArgsReadback->Unlock();

			// Independent sum of ClusterSplatCounts
			uint32 ReadVisibleCount = *(const uint32*)PrefixTest_NewTotalReadback->Lock(UintStride);
			PrefixTest_NewTotalReadback->Unlock();

			uint32 DirectSum = 0;
			if (ReadVisibleCount > 0)
			{
				uint32 ReadCount = FMath::Min(ReadVisibleCount, ClusterTest_MaxReadbackClusters);
				const uint32* Counts = (const uint32*)PrefixTest_OldDrawArgsReadback->Lock(ReadCount * UintStride);
				for (uint32 i = 0; i < ReadCount; i++)
				{
					DirectSum += Counts[i];
				}
				PrefixTest_OldDrawArgsReadback->Unlock();
			}

			bool bPass = (PrefixSumTotal == DirectSum);
			UE_LOG(LogTemp, Log,
				TEXT("[GS Cluster] Stage2 TEST: PrefixSumTotal=%u vs DirectSum(ClusterSplatCounts)=%u, VisibleClusters=%u -> %s"),
				PrefixSumTotal, DirectSum, ReadVisibleCount,
				bPass ? TEXT("PASS") : TEXT("FAIL <<<"));

			PrefixTest_NewDrawArgsReadback.Reset();
			PrefixTest_NewTotalReadback.Reset();
			PrefixTest_OldDrawArgsReadback.Reset();
			bPrefixTestWaitingForReadback = false;
		}
	}
}

// ============================================================================
// Cluster-based CalcViewData: DispatchClusterCalcViewData
// ============================================================================

static TAutoConsoleVariable<int32> CVarClusterCalcViewDataTest(
	TEXT("gs.ClusterCalcViewDataTest"), 0,
	TEXT("Set to 1 to test cluster CalcViewData (compares valid splat counts with old path)"));

void FGlobalSplatBufferManager::DispatchClusterCalcViewData(
	FRHICommandListImmediate& RHICmdList,
	const FSceneView& View,
	FGaussianGlobalAccumulator* GlobalAccumulator,
	uint32 MaxRenderBudget)
{
	...
	SCOPED_DRAW_EVENT(RHICmdList, GaussianSplatClusterCalcViewData);

	TShaderMapRef<FClusterCalcViewDataCS> CalcViewShader(GetGlobalShaderMap(GMaxRHIFeatureLevel));
	if (!CalcViewShader.IsValid())
	{
		UE_LOG(LogTemp, Warning, TEXT("[GS Cluster] ClusterCalcViewData shader not valid"));
		return;
	}
	...
	if (!ShadowClusterViewDataBuffer.IsValid())
	{
		...
		ShadowClusterViewDataBufferUAV = RHICmdList.CreateUnorderedAccessView(
			ShadowClusterViewDataBuffer,
	...
	RHICmdList.Transition(FRHITransitionInfo(ClusterOutputOffsetsBuffer, ERHIAccess::Unknown, ERHIAccess::SRVCompute));
	RHICmdList.Transition(FRHITransitionInfo(GlobalPackedSplatBuffer, ERHIAccess::Unknown, ERHIAccess::SRVCompute));
	RHICmdList.Transition(FRHITransitionInfo(GlobalSHBuffer, ERHIAccess::Unknown, ERHIAccess::SRVCompute));
	RHICmdList.Transition(FRHITransitionInfo(ProxyMetadataBuffer, ERHIAccess::Unknown, ERHIAccess::SRVCompute));
	RHICmdList.Transition(FRHITransitionInfo(ProxyRenderParamsBuffer, ERHIAccess::Unknown, ERHIAccess::SRVCompute));
	...
	FClusterCalcViewDataCS::FParameters Params;
	Params.VisibleClusterWorkList = VisibleClusterWorkListSRV;
	Params.ClusterOutputOffsetsBuffer = ClusterOutputOffsetsBufferSRV;
	Params.GlobalPackedSplatBuffer = GlobalPackedSplatBufferSRV;
	Params.GlobalSHBuffer = GlobalSHBufferSRV;
	Params.ProxyMetadataBuffer = ProxyMetadataBufferSRV;
	...
	Params.ScreenSize = FVector2f(ViewRect.Width(), ViewRect.Height());
	Params.FocalLength = FVector2f(
		ProjMatrix.M[0][0] * ViewRect.Width() * 0.5f,
		ProjMatrix.M[1][1] * ViewRect.Height() * 0.5f);
	Params.MaxRenderBudget = MaxRenderBudget;

	SetComputePipelineState(RHICmdList, CalcViewShader.GetComputeShader());
	SetShaderParameters(RHICmdList, CalcViewShader, CalcViewShader.GetComputeShader(), Params);
	RHICmdList.DispatchIndirectComputeShader(ClusterCalcViewDataIndirectArgs, 0);
	...
	// Compare valid splat counts (ClipPosition.w > 0) between new and old.
	// ---------------------------------------------------------------
	int32 TestVal = CVarClusterCalcViewDataTest.GetValueOnRenderThread();

	if (TestVal == 1 && !bCalcViewTestWaitingForReadback)
	{
		bCalcViewTestWaitingForReadback = true;

		// Read VisibleClusterCount to know TotalVisible
		CalcViewTest_CountReadback = MakeUnique<FRHIGPUBufferReadback>(TEXT("CalcViewTestCount"));
		RHICmdList.Transition(FRHITransitionInfo(VisibleClusterCountBuffer, ERHIAccess::Unknown, ERHIAccess::CopySrc));
		CalcViewTest_CountReadback->EnqueueCopy(RHICmdList, VisibleClusterCountBuffer, sizeof(uint32));

		// Read first 100 entries from new buffer for validation
		uint32 SampleCount = FMath::Min(TotalSplatCount, 100u);
		CalcViewTest_TotalVisible = SampleCount;

		CalcViewTest_NewReadback = MakeUnique<FRHIGPUBufferReadback>(TEXT("CalcViewTestNew"));
		RHICmdList.Transition(FRHITransitionInfo(ShadowClusterViewDataBuffer, ERHIAccess::SRVCompute, ERHIAccess::CopySrc));
		CalcViewTest_NewReadback->EnqueueCopy(RHICmdList, ShadowClusterViewDataBuffer, SampleCount * ViewDataStride);

		// Read first 100 entries from old buffer
		CalcViewTest_OldReadback = MakeUnique<FRHIGPUBufferReadback>(TEXT("CalcViewTestOld"));
		RHICmdList.Transition(FRHITransitionInfo(GlobalAccumulator->GlobalViewDataBuffer, ERHIAccess::Unknown, ERHIAccess::CopySrc));
		CalcViewTest_OldReadback->EnqueueCopy(RHICmdList, GlobalAccumulator->GlobalViewDataBuffer, SampleCount * ViewDataStride);
	}
	else if (bCalcViewTestWaitingForReadback)
	{
		if (CalcViewTest_CountReadback && CalcViewTest_CountReadback->IsReady()
			&& CalcViewTest_NewReadback && CalcViewTest_NewReadback->IsReady()
			&& CalcViewTest_OldReadback && CalcViewTest_OldReadback->IsReady())
		{
			uint32 VisibleClusters = *(const uint32*)CalcViewTest_CountReadback->Lock(sizeof(uint32));
			CalcViewTest_CountReadback->Unlock();

			uint32 SampleCount = CalcViewTest_TotalVisible;
			const FGaussianSplatViewData* NewData = (const FGaussianSplatViewData*)CalcViewTest_NewReadback->Lock(SampleCount * ViewDataStride);
			const FGaussianSplatViewData* OldData = (const FGaussianSplatViewData*)CalcViewTest_OldReadback->Lock(SampleCount * ViewDataStride);
			...
			UE_LOG(LogTemp, Log,
				TEXT("[GS Cluster] Stage3 TEST: VisibleClusters=%u, SampleSize=%u, NewValid=%u OldValid=%u -> %s"),
				VisibleClusters, SampleCount, NewValidCount, OldValidCount,
				(NewValidCount > 0) ? TEXT("PRODUCING OUTPUT") : TEXT("NO OUTPUT <<<"));

			// Log first 3 sample entries from new buffer
			for (uint32 i = 0; i < FMath::Min(SampleCount, 3u); i++)
			{
				UE_LOG(LogTemp, Log,
					TEXT("[GS Cluster]   New[%u]: clip=(%.2f,%.2f,%.2f,%.2f) cluster=%u"),
					i, NewData[i].ClipPosition.X, NewData[i].ClipPosition.Y,
					NewData[i].ClipPosition.Z, NewData[i].ClipPosition.W,
					NewData[i].ClusterID);
			}

			CalcViewTest_NewReadback->Unlock();
			CalcViewTest_OldReadback->Unlock();
			CalcViewTest_CountReadback.Reset();
			CalcViewTest_NewReadback.Reset();
			CalcViewTest_OldReadback.Reset();
			bCalcViewTestWaitingForReadback = false;
		}
	}
}
