...
/**
 * Reset shader: clears VisibleClusterCount to 0.
 */
class FBuildVisibleClusterWorkListResetCS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FBuildVisibleClusterWorkListResetCS);
	SHADER_USE_PARAMETER_STRUCT(FBuildVisibleClusterWorkListResetCS, FGlobalShader);
...
/**
 * Leaf cluster pass: scans visible leaf clusters and appends to work list.
 * Uses binary search on ProxyMetadataBuffer.GlobalLeafClusterEnd.
 */
class FBuildVisibleClusterWorkListCS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FBuildVisibleClusterWorkListCS);
	SHADER_USE_PARAMETER_STRUCT(FBuildVisibleClusterWorkListCS, FGlobalShader);
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_SRV(StructuredBuffer<uint>, ShadowClusterVisibilityBitmap)
		SHADER_PARAMETER_SRV(StructuredBuffer<uint>, ShadowSelectedClusterBuffer)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, VisibleClusterWorkList)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, VisibleClusterCountBuffer)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, ClusterSplatCountsBuffer)
		SHADER_PARAMETER(uint32, ProxyCount)
		SHADER_PARAMETER(uint32, TotalLeafClusterCount)
		SHADER_PARAMETER(uint32, UseLODRendering)
	END_SHADER_PARAMETER_STRUCT()
...
/**
 * LOD parent cluster pass: scans LOD clusters from ShadowLODClusterBuffer.
 * Uses binary search on cluster index range to find proxy ownership.
 */
class FBuildVisibleClusterWorkListLODCS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FBuildVisibleClusterWorkListLODCS);
	SHADER_USE_PARAMETER_STRUCT(FBuildVisibleClusterWorkListLODCS, FGlobalShader);

	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER_SRV(StructuredBuffer<FGaussianGPUCluster>, GlobalClusterBuffer)
		SHADER_PARAMETER_SRV(StructuredBuffer<FProxyGPUMetadata>, ProxyMetadataBuffer)
		SHADER_PARAMETER_SRV(StructuredBuffer<uint>, ShadowLODClusterBuffer)
		SHADER_PARAMETER_SRV(StructuredBuffer<uint>, ShadowLODClusterCountBuffer)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, VisibleClusterWorkList)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, VisibleClusterCountBuffer)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint>, ClusterSplatCountsBuffer)
		SHADER_PARAMETER(uint32, ProxyCount)
